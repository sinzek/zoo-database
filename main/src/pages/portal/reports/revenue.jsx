	...
	const loadReport = async () => {
		...
		try {
			...
			if (res.success) {
				...
				console.log(
					'Sample report object keys:',
					normalized[0] ? Object.keys(normalized[0]) : 'no items'
				);
		...
		return (
			<div className='portal-revenue-page'>
				<p className='error-message'>
					Reports are available to managers and above.
				</p>
			</div>
		);
	}
					...
					<div className='form-group'>
						...
						<div className='business-checkbox-container'>
							{businesses.map((b) => (
								<label
									key={b.businessId}
									className='business-checkbox-item'
								>
									<input
										type='checkbox'
										value={b.businessId}
										checked={selectedBusinessIds.includes(
											b.businessId
										)}
										onChange={(e) => {
											if (e.target.checked) {
